...
* Toolchain : GnuMake (configured via `gnumake`)
* SoftwareComponent : A Makeifle project than can produce on or more components
* BinaryComponents : Each artifact that is built by the Makefile
* Binary Tasks
** cleanXyz : make clean FLAG1=value1 -x
** buildXyz : make defaultTargets
...
NOTE: Buildscripts written to versions prior to 2.x are not compatible with the 2.0+. Please read the notes on upgrading
...
----
plugins {
  id 'org.ysb33r.gnumake' version '2.0'
}
----

== Global configuration

Defaults to be set can be set for all `GnuMakeBuild` tasks in the `gnumake` model space.
...
The layout is the same, excep tthe `gnumake` block now resides in the `model` block

.Pre-2.0 Global configuration
[source,groovy]
----
gnumake {
    executable '/path/to/make'
}
----
...
A number of classes have moved. Please update your imports if you had customised builds.

|===
| Class | New Package | Old Package
| `GnuMakeBuild` | `org.ysb33r.gradle.gnumake.tasks` | `org,ysb33r.gradle.gnumake`
| `GnuMakePlugin` | `org.ysb33r.gradle.gnumake.model` | `org,ysb33r.gradle.gnumake`
| `GnuMakeExtension` | `org.ysb33r.gradle.gnumake.model` | `org,ysb33r.gradle.gnumake`
|===
